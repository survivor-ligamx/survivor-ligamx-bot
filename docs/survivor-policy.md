...
La respuesta incluye `estados_dp_evaluados` para vigilar la complejidad. En calendarios largos se usa un horizonte móvil corto de bloques exactos para mantener tiempos de respuesta acotados en producción; los equipos utilizados y el estado de la vida se conservan entre bloques.

La ruta web predeterminada calcula fuerzas desde el snapshot histórico local y deja la calibración walk-forward como opción explícita, evitando llamadas históricas secuenciales dentro de la petición. Las transiciones con vida disponible y consumida tienen cobertura de regresión.
