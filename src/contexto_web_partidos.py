...
_PREVIA_ACCIONABLE = ("lesion", "lesionado", "sera baja", "causa baja", "baja confirmada", "ausencia confirmada", "no estara", "se pierde el partido", "suspension", "suspendido", "alineacion probable", "alineacion confirmada", "rotacion", "convocados", "convocatoria", "parte medico", "regresa", "disponible", "podria debutar", "puede debutar", "visa de trabajo")
_PREVIA_PARTIDO = ("previa", " vs ", " contra ", "recibe a", "visita a", "enfrenta a", "partido", "jornada")
_POST_PARTIDO = ("resultado", "resumen", "cronica", "gana", "gano", "victoria", "vencio", "derroto", "empato", "empate", "final")
...
def _asegurar_tabla() -> None:
    ...
    with db.get_db() as conn:
        cur = conn.cursor()
        cur.execute(f"""CREATE TABLE IF NOT EXISTS {_TABLA} (clave TEXT PRIMARY KEY, jornada INTEGER NOT NULL, local TEXT NOT NULL, visitante TEXT NOT NULL, fase TEXT NOT NULL, fecha_partido TEXT, resumen TEXT, hallazgos TEXT, fuentes TEXT, proveedores TEXT, actualizado_en TEXT NOT NULL, expira_en TEXT NOT NULL)""")
        conn.commit()
...
def clasificar_evento(resultado: Mapping[str, Any], local: str, visitante: str) -> Dict[str, Any]:
    ...
    if tipo == "ALINEACION_CONFIRMADA" or (nivel == "oficial" and tipo in {"BAJA_CONFIRMADA", "SUSPENSION", "CONVOCATORIA", "DISPONIBLE"}):
        confirmacion = "CONFIRMADA"
    return {"equipo": equipo, "tipo": tipo, "confirmacion": confirmacion, "titularidad": "CONFIRMADA" if tipo == "ALINEACION_CONFIRMADA" else "NO_CONFIRMADA", "fecha": str(resultado.get("fecha") or "")[:40], "url": url, "proveedor": str(resultado.get("proveedor") or ""), "dominio": urlparse(url).netloc.lower().removeprefix("www."), "nivel_fuente": nivel, "titulo": titulo}
...
def _leer_cache(local: str, visitante: str, fase: str, jornada: Optional[int] = None) -> Optional[Dict[str, Any]]:
    try:
        ...
        with db.get_db() as conn:
            cur = conn.cursor()
            cur.execute(f"SELECT jornada, fecha_partido, resumen, hallazgos, fuentes, proveedores, actualizado_en, expira_en FROM {_TABLA} WHERE clave={db.PH}", (_clave(local, visitante, fase, jornada),))
            fila = cur.fetchone()
        if not fila:
            return None
        ...
        resultados = [{"titulo": t, "url": fuentes[i] if i < len(fuentes) else "", "proveedor": "cache"} for i, t in enumerate(hallazgos)]
        eventos = [clasificar_evento(r, local, visitante) for r in resultados]
        return {"disponible": bool(fila[2]), "jornada": int(fila[0]), "fecha_partido": str(fila[1] or ""), "local": local, "visitante": visitante, "fase": fase, "resumen": str(fila[2] or ""), "hallazgos": hallazgos, "fuentes": fuentes, "proveedores": _decodificar_json(fila[5]), "eventos": eventos, "estado": _estado_eventos(eventos), "actualizado_en": str(fila[6]), "frescura": _frescura(fila[6]), "cache": True}
    except Exception:
        logger.debug("No se pudo leer la caché de contexto por jornada", exc_info=True)
        return None


def _guardar_cache(local: str, visitante: str, fase: str, contexto: Mapping[str, Any], jornada: Optional[int] = None, fecha_partido: Optional[str] = None) -> None:
    try:
        from src import database as db
        _asegurar_tabla()
        ahora = _ahora()
        ttl = ((_TTL_POST_HORAS if fase == "post" else _TTL_PREVIA_HORAS) if contexto.get("resumen") else _TTL_SIN_RESULTADOS_HORAS)
        valores = (_clave(local, visitante, fase, jornada), _jornada_valida(jornada), local, visitante, fase, str(fecha_partido or "")[:10], str(contexto.get("resumen") or ""), json.dumps(contexto.get("hallazgos") or [], ensure_ascii=False), json.dumps(contexto.get("fuentes") or [], ensure_ascii=False), json.dumps(contexto.get("proveedores") or [], ensure_ascii=False), ahora.isoformat(), (ahora + timedelta(hours=ttl)).isoformat())
        with db.get_db() as conn:
            cur = conn.cursor()
            cur.execute(f"""INSERT INTO {_TABLA} (clave,jornada,local,visitante,fase,fecha_partido,resumen,hallazgos,fuentes,proveedores,actualizado_en,expira_en) VALUES ({', '.join([db.PH] * 12)}) ON CONFLICT (clave) DO UPDATE SET jornada=excluded.jornada,local=excluded.local,visitante=excluded.visitante,fase=excluded.fase,fecha_partido=excluded.fecha_partido,resumen=excluded.resumen,hallazgos=excluded.hallazgos,fuentes=excluded.fuentes,proveedores=excluded.proveedores,actualizado_en=excluded.actualizado_en,expira_en=excluded.expira_en""", valores)
            conn.commit()
    except Exception:
...
def _historial_equipo(equipo: str, jornada_actual: int, limite: int = 2) -> List[Dict[str, Any]]:
    try:
        ...
        with db.get_db() as conn:
            cur = conn.cursor()
            cur.execute(f"SELECT jornada,resumen FROM {_TABLA} WHERE fase={db.PH} AND jornada < {db.PH} AND (local={db.PH} OR visitante={db.PH}) AND resumen IS NOT NULL AND resumen <> '' ORDER BY jornada DESC LIMIT {db.PH}", ("post", jornada_actual, equipo, equipo, max(1, limite)))
            filas = cur.fetchall()
...
def _item(titulo: Any, url: Any, texto: Any, fecha: Any, proveedor: str, local: str = "", visitante: str = "", fase: str = "previa") -> Optional[Dict[str, str]]:
    item = {"titulo": " ".join(str(titulo or "").split())[:220], "url": str(url or "").strip()[:500], "texto": " ".join(str(texto or "").split())[:700], "fecha": str(fecha or "")[:40], "proveedor": proveedor}
...
def _tavily(query: str, local: str = "", visitante: str = "", fase: str = "previa") -> List[Dict[str, str]]:
    ...
    if not key or not _reclamar_credito():
        return []
    payload: Dict[str, Any] = {"api_key": key, "query": query, "topic": "news", "search_depth": "basic", "max_results": _MAX_RESULTADOS, "days": 30, "include_answer": False, "include_raw_content": False}
    ...
    for raw in respuesta.json().get("results", []):
        if isinstance(raw, dict) and (normalizado := _item(raw.get("title"), raw.get("url"), raw.get("content"), raw.get("published_date"), "tavily", local, visitante, fase)):
...
def _gnews(query: str, local: str = "", visitante: str = "", fase: str = "previa") -> List[Dict[str, str]]:
    ...
    if not key or not _reclamar_credito():
        return []
    respuesta = requests.get("https://gnews.io/api/v4/search", params={"q": query, "lang": "es", "country": "mx", "max": _MAX_RESULTADOS, "apikey": key}, timeout=20)
    respuesta.raise_for_status()
    salida = []
    for raw in respuesta.json().get("articles", []):
        if isinstance(raw, dict) and (normalizado := _item(raw.get("title"), raw.get("url"), raw.get("description") or raw.get("content"), raw.get("publishedAt"), "gnews", local, visitante, fase)):
            salida.append(normalizado)
    return salida
...
def _serper(query: str, local: str = "", visitante: str = "", fase: str = "previa") -> List[Dict[str, str]]:
    ...
    if not key or not _reclamar_credito():
        return []
    respuesta = requests.post("https://google.serper.dev/news", headers={"X-API-KEY": key, "Content-Type": "application/json"}, json={"q": query, "gl": "mx", "hl": "es-419", "num": _MAX_RESULTADOS}, timeout=20)
    respuesta.raise_for_status()
    salida = []
    for raw in respuesta.json().get("news", []):
        if isinstance(raw, dict) and (normalizado := _item(raw.get("title"), raw.get("link"), raw.get("snippet"), raw.get("date"), "serper", local, visitante, fase)):
            salida.append(normalizado)
    return salida


def _consulta(local: str, visitante: str, fase: str, jornada: Optional[int] = None, fecha_partido: Optional[str] = None) -> str:
...
def _buscar(local: str, visitante: str, fase: str, jornada: Optional[int] = None, fecha_partido: Optional[str] = None) -> List[Dict[str, str]]:
...
def _resumir(resultados: Sequence[Mapping[str, str]], local: str = "", visitante: str = "") -> Dict[str, Any]:
    ...
    return {"disponible": bool(resumen), "resumen": resumen, "hallazgos": hallazgos, "fuentes": fuentes, "proveedores": proveedores, "eventos": eventos, "estado": _estado_eventos(eventos)}


def investigar_partido(local: str, visitante: str, fase: str = "previa", forzar: bool = False, jornada: Optional[int] = None, fecha_partido: Optional[str] = None) -> Dict[str, Any]:
    ...
    contexto = {"local": local, "visitante": visitante, "fase": fase, "jornada": _jornada_valida(jornada), "fecha_partido": str(fecha_partido or "")[:10], "actualizado_en": ahora, "frescura": "FRESCA", "cache": False, **_resumir(_buscar(local, visitante, fase, jornada, fecha_partido), local, visitante)}
...
def contextos_para_plan(plan: Mapping[str, Any], limite: int = 3) -> List[Dict[str, Any]]:
    ...
    for paso in pasos[:max(0, limite)]:
        ...
        local, visitante = (equipo, rival) if str(paso.get("condicion") or "").lower().startswith("local") else (rival, equipo)
        previa = _leer_cache(local, visitante, "previa", jornada)
        if previa and previa.get("disponible"):
            salida.append({"jornada": jornada, "equipo": equipo, "resumen": str(previa.get("resumen") or "")[:500], "fuentes": previa.get("fuentes", []), "estado": previa.get("estado", "PROVISIONAL"), "frescura": previa.get("frescura", "DESCONOCIDA")})
    return salida
...
def actualizar_contexto_automatico(hoy: Optional[date] = None) -> Dict[str, Any]:
    ...
    for bloque in calendario:
        ...
        for partido in _partidos_bloque(bloque):
            tareas.append((jornada, "previa", str(partido.get("home_team") or ""), str(partido.get("away_team") or ""), inicio.isoformat()))
    ...
    for bloque in completadas:
        ...
        for partido in _partidos_bloque(bloque):
            tareas.append((jornada, "post", str(partido.get("home_team") or ""), str(partido.get("away_team") or ""), fin.isoformat() if fin else ""))
    try:
    ...
    for jornada, fase, local, visitante, fecha_partido in tareas:
        ...
        if consultas >= limite:
            break
    return {"ejecutado": True, "jornadas": sorted(jornadas), "partidos_revisados": revisados, "consultas": consultas, "disponibles": disponibles, "backfill_completo": consultas == 0}
